...
try {
    ...
    $occupiedQuery = "SELECT COUNT(DISTINCT room_id) as cnt FROM bookings WHERE status = 'checked-in'";
    ...
    // 5. Recent Bookings
    $bookingsQuery = "SELECT b.id, u.name as guest_name, rt.name as room_type, b.check_in_date, b.check_out_date, b.total_price, b.status 
                      FROM bookings b 
                      INNER JOIN users u ON b.guest_id = u.id
                      LEFT JOIN rooms r ON b.room_id = r.id
                      LEFT JOIN room_types rt ON r.room_type_id = rt.id OR b.room_id IS NULL AND rt.id = 1 -- fallback
