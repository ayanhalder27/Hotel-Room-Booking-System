...
function loadDashboardStats(){
    getData("../Controler/hk_dashboard.php?action=dashboard_stats", function(data){
        if(data.success){
...
function loadTodayCheckouts(){
    getData("../Controler/hk_dashboard.php?action=today_checkouts", function(data){
        let output = "";
...
function loadUpcomingCheckins(){
    getData("../Controler/hk_dashboard.php?action=upcoming_checkins", function(data){
        let output = "";
...
function loadUrgentTasks(){
    getData("../Controler/hk_dashboard.php?action=urgent_tasks", function(data){
        let output = "";
        ...
        if(data.success && data.rows.length > 0){
            ...
            for(let i = 0; i < data.rows.length; i++){
                ...
                // 6th column: Action
                output += "<td>";
                output += "<a href='hk_tasks.html' class='table-action'>Update</a>";
                output += "</td>";
